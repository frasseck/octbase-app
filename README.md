...
- **Plain-DOM JS frontends** (`octbase-frontend/`, `octbase-mobile/`) — no framework, no JSX, no client state library; **ES modules built by Vite** from an npm workspace. The bundler-free half of the stance was retired on 2026-07-30 — decision record in `docs/architecture.md` §5.2, staged migration in `prompts/37b_octbase-frontend-build-step.md`
...
repository (`ocete.ch`); it is no longer part of this repo.
...
Build and serve the frontend. A static file server pointed at the source tree no
longer works: `index.html` loads one ES module entry whose imports resolve
...
and is no longer part of this repo or its `podman-compose.yml`. See that repo for
...
- **Whole-project ZIP export/import** — `GET /projects/{id}/export` bundles a project (tasks, pages, attachments, plus the releases, sprints, boards, categories and templates the tasks are planned in, and each task's placement in them) into a ZIP; import it into an existing project (`POST /projects/{id}/import`) or as a new project (`POST /projects/import`). Memberships, SCM connections and the activity log are deliberately excluded. `formatVersion` stays 1 — every section added after the first release is optional, in both directions
- Imports support `dryRun=true` — preview counts without writing to the database

> The legacy platform-wide admin importers (`/admin/import/jira`,
> `/admin/import/confluence`) were removed for security reasons. For Jira,
...
|---|---|---|---|
| `pre-commit` | `scripts/security-sweep.sh` | Deterministic, no-stack security sweep: frontend integrity guards (innerHTML, shared-module drift, JS syntax) + backend/frontend regression greps (no `math/rand`/`InsecureSkipVerify`/`os-exec`/unescaped `X-Forwarded-For`/SVG-upload/`eval`/inline `<script>`/token-in-URL/secrets-in-storage) + `gofmt`. Ambiguous patterns are printed as non-blocking warnings. Also restamps SPA asset hashes. | ~1–2s |
...
ESLint replaced a hand-maintained `node --check` loop at 37b stage 6: the build
already answers "does this parse" for everything in the module graph, and ESLint
answers it for everything outside it (the classic scripts, the unit tests, the
`.mjs` guards) while also catching a name used with no import behind it and an
import that outlived its caller. Both found real bugs on their first run.
...
**Set `OCTBASE_UI_URL` at the built app.** Since 37b stage 6 the desktop suite drives `octbase-frontend/dist/` over HTTP, which is both closer to what a user gets and the only way the suite can see a CORS or CSP regression at all:
...
> ⚠️ **There is no default, on purpose.** `conftest.py` used to fall back to `octbase-frontend/index.html` via `file://`, which worked while the app was ordered classic scripts. It is now a `<script type="module">` entry importing bare specifiers (`@octbase/shared/…`), and a browser can neither resolve those nor `import` at all from a `file://` origin — so that fallback produced a blank page and a wall of fixture timeouts naming nothing. It was removed: a run with no `OCTBASE_UI_URL` now **skips the desktop tests** with a message naming the variable and giving the recipe, and **fails** rather than skips when `CI` is set, so a job that forgets the variable cannot report green on a run that drove nothing. Serving the *source* tree with `python3 -m http.server` fails for the original reason regardless: the bare specifiers need the bundler.
