...
- **A database ahead of the migrations on disk now fails with an actionable
  error instead of an opaque one.** Startup compares the recorded
  `schema_migrations` version against the highest migration file present, and a
  database recording a version this build has no file for stops with the two
  versions, why it cannot be migrated automatically, and a pointer to the
  stamping procedure in `docs/operations.md`. golang-migrate's own diagnosis for
  that state is `no migration found for version 38: read down for version 38` —
  it is looking for the *down* file to plan from, so it reads as a corrupt or
  missing file rather than as a database from before a history rewrite, and
  during the `001_baseline` rollout it was first triaged as degraded health
  rather than the hard startup failure it is. Startup still refuses to proceed
  and nothing self-repairs: stamping is an assertion about a schema
  golang-migrate cannot verify and never re-checks, and which version the
  instance stopped at changes the procedure, so it stays an operator decision.
  Covered by the first test to run migrations against a database that is not
  empty — `testutil` executes migration SQL directly rather than through
  golang-migrate, so the migrate path every upgrade deploy takes had no test
  coverage at all and a fresh-database CI run could not have caught this.
